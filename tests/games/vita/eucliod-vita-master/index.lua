...
keymap={}; keymap2={}
keymap[KEY_A]=0; keymap2[0] = KEY_A 
keymap[KEY_B]=1; keymap2[1] = KEY_B
keymap[KEY_Y]=2; keymap2[2] = KEY_Y 
keymap[KEY_X]=3; keymap2[3] = KEY_X 
keymap[KEY_L]=4; keymap2[4] = KEY_L 
keymap[KEY_R]=5; keymap2[5] = KEY_R 
keymap[KEY_ZL]=6; keymap2[6] = KEY_ZL 
keymap[KEY_ZR]=7; keymap2[7] = KEY_ZR 

local KEY_FIRE = KEY_Y
local KEY_FIRE2 = KEY_B
local KEY_WEAPON = KEY_X
local KEY_FOCUS = KEY_R

if System.doesFileExist(rotdir.."/config.dat") then
	local file_config = io.open(rotdir.."/config.dat",FREAD)
...
img_load = Graphics.loadImage(appdir..'/load.png')
...
DEBUG = false
debugval1 = 0
local new3ds = System.getModel()
if (new3ds == 2 or new3ds == 4) and not (Controls_check(pad,KEY_Y)) then
	new3ds = true
	System.setCpuSpeed(804)
	img_new3ds_mode = Graphics.loadImage(appdir..'/new3ds_mode.png')
else
	new3ds = false
end
...
if Controls_check(pad,KEY_X) then
	DEBUG = true
	c_line = c_green
end
...
v_frame = 0 --currently recording frame

playwav = {} --ensures duplicate instances of sounds are only played once each per step
playwav.b01 = false
playwav.b02 = false
playwav.b03 = false
playwav.b04 = false
		...
		while i<enemylist.size do 
			...
			elseif t == 201 then --boss 01
				...
				elseif bhp > 0.33*stage_boss_hp_max then
					...
					local ang = 0.1+bit.rshift(difficulty, 7) --this apparently means divided by 16
					if ang*cl < sw then
