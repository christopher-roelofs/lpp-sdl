Bug = {}
bugImage = Graphics.loadImage("app0:/Sprites/Bug.png");

function Bug:New(x,y, created, enemyGeneratorParent, orgin)
  bug = {}
  setmetatable(bug,self)
  self.__index = self
  
  bug.health = 15
  bug.x = x
  bug.y = y
  bug.tintColor = Color.new(255,255,255)
  bug.hitConfirmTimer = 0;
  bug.directionFacing = "north"
  bug.canDecideMoveDirection = false
  bug.moveDirection = "north"
  bug.moveDirectionTimer = 0;
  bug.speed = 8
  bug.createdByEnemyGenerator = created
  bug.parent = enemyGeneratorParent
  bug.isDead = false
  bug.power = 5
  bug.orgin = orgin
  return bug
...
function Bug:Draw()
  ...
  if(self.directionFacing == "north") then
    Graphics.drawImageExtended(self.x - cameraX,self.y - cameraY, 0,0,32, 32,1,1,bugImage,self.tintColor)
  end
  
  if(self.directionFacing == "south") then
     Graphics.drawImageExtended(self.x - cameraX,self.y + 32 - cameraY, 0,0,32, 32,1,-1,bugImage,self.tintColor)
  end
  
  if(self.directionFacing == "east") then
     Graphics.drawImageExtended(self.x - cameraX,self.y - cameraY, 32,0,32, 32,1,1,bugImage,self.tintColor)
  end
  
  if(self.directionFacing == "west") then
    Graphics.drawImageExtended(self.x + 32 - cameraX,self.y - cameraY, 32,0,32, 32,-1,1,bugImage,self.tintColor)
  end
  
  
end

function Bug:CheckDirectionToMove()
...
function Bug:CheckRelationToPlayer()
...
function Bug:CheckMoveBoundries()
if(self.x < 0) then
  self.x = 0
end
...
function Bug:DecideMoveDirection()
...
function Bug:Raycast(direction, speed)
  ...
  function Bug:RaycastPlayer(direction, speed) 
...
function Bug:RayCastEnemy(direction, speed)
   if(direction == "west") then
...
function Bug:TakeDamage(damage)
  ...
  if(self.health <= 0) then
    ...
    if(self.createdByEnemyGenerator == true) then
...
function Bug:GetCorrectDirection()
  if(self.moveDirection ~= "") then 
  self.directionFacing = self.moveDirection
  end
end


 function Bug:NotOnScreen()
