Bat = {}
batImage = Graphics.loadImage("app0:/Sprites/Bat.png")

function Bat:New(x,y,created,enemyGeneratorParent, orgin)
  bat = {}
  ...
  bat.hitConfirmTimer = 0
  bat.tintColor = Color.new(255,255,255)
  bat.isDead = false
  bat.canDecideMoveDirection = true
  bat.moveDirectionTimer = 0;
  bat.moveDirection = ""
  bat.power = 3
  bat.orgin = orgin
  
  
  bat.isGeneratorCreated = created
  bat.generatorParent = enemyGeneratorParent
  return bat
end
...
function Bat:Draw()
  ...
  if(self.animationFrame == 1) then
    Graphics.drawPartialImage(self.x - cameraX, self.y - cameraY, 0, 0, 24, 24, batImage, self.tintColor)
  end
  
  if(self.animationFrame == 2) then
    Graphics.drawPartialImage(self.x - cameraX, self.y - cameraY, 24, 0, 24, 24, batImage, self.tintColor)
  end
  
  if(self.animationFrame == 3) then
    Graphics.drawPartialImage(self.x - cameraX, self.y - cameraY, 0, 24, 24, 24, batImage, self.tintColor)
  end
  
  if(self.animationFrame == 4) then
    Graphics.drawPartialImage(self.x - cameraX, self.y - cameraY, 24, 24, 24, 24, batImage, self.tintColor)
  end
  
  if(self.animationFrame == 5) then
    Graphics.drawPartialImage(self.x - cameraX, self.y - cameraY, 0, 48, 24, 24, batImage, self.tintColor)
  end
...
function Bat:CheckMoveBoundries()
if(self.x < 0) then
  self.x = 0
end
...
function Bat:GetDimensions()
  
  return {self.x, self.x + 24, self.y + 4, self.y + 16}
end



function Bat:DecideMoveDirection()
...
function Bat:CheckRelationToPlayer()
...
function Bat:RaycastPlayer(direction, speed) 
...
function Bat:RayCastEnemy(direction, speed)
   if(direction == "west") then
...
function Bat:Raycast(direction, speed)
 ...
 table.insert(onScreenTextPopUpObj, #onScreenTextPopUpObj + 1, OnScreenTextPopUps:New(self.x,self.y, tostring(damage),Color.new(255,255,255)))
   if(self.health <= 0) then
     ...
     if(self.isGeneratorCreated == true) then
     self.generatorParent:RemoveOneEnemy()  
     end
