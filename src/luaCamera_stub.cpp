...
#-----------------------------------------------------------------------------------------------------------------------#
#-----------------------------------------------------------------------------------------------------------------------#
#- SDL Port: Camera Module Stub Implementation (no OpenCV) ------------------------------------------------------------#
#-----------------------------------------------------------------------------------------------------------------------*/
...
void initCam(uint8_t type, uint8_t res, uint8_t fps) {
    ...
    // Create a dummy texture for camera output
...
// Camera.init(device, resolution, framerate) - stub
static int lua_caminit(lua_State *L) {
    ...
    // Log that camera is running in stub mode
    SDL_Log("Camera initialized in stub mode (no OpenCV support)");
    
    return 0;
}

// Camera.getOutput() - Return dummy texture
static int lua_camoutput(lua_State *L) {
    int argc = lua_gettop(L);
    if (argc != 0) return luaL_error(L, "wrong number of arguments.");
    if (!isCamOn) return luaL_error(L, "camera has not been initialized.");
    
    // Create a dummy texture if not already created
    if (!cam_texture->texture) {
        cam_texture->texture = SDL_CreateTexture(g_renderer, SDL_PIXELFORMAT_RGBA8888,
                                               SDL_TEXTUREACCESS_TARGET,
                                               cam_width, cam_height);
        
        if (cam_texture->texture) {
            // Fill with a placeholder pattern
            SDL_SetRenderTarget(g_renderer, (SDL_Texture*)cam_texture->texture);
            SDL_SetRenderDrawColor(g_renderer, 64, 64, 64, 255);
            SDL_RenderClear(g_renderer);
            
            // Draw a simple pattern to indicate stub mode
            SDL_SetRenderDrawColor(g_renderer, 128, 128, 128, 255);
            for (int i = 0; i < cam_width; i += 32) {
                SDL_RenderDrawLine(g_renderer, i, 0, i, cam_height);
            }
            for (int i = 0; i < cam_height; i += 32) {
                SDL_RenderDrawLine(g_renderer, 0, i, cam_width, i);
            }
            
            // Draw text indicating no camera
            SDL_SetRenderDrawColor(g_renderer, 255, 255, 255, 255);
            const char* msg = "NO CAMERA";
            int text_w = strlen(msg) * 8;
            int text_h = 16;
            SDL_Rect text_rect = {
                (cam_width - text_w) / 2,
                (cam_height - text_h) / 2,
...
// Camera.switchDevice() - Stub implementation
...
// Camera.term() - Clean up camera (stub)
...
// Camera setting functions (stubs)
