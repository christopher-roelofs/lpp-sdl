...
static void update_input_state() {
    ...
    if (controllers[0] && SDL_GameControllerGetAttached(controllers[0])) {
        ...
        // D-Pad mapping - only for 3DS compatibility mode
        extern lpp_compat_mode_t g_compat_mode;
        if (g_compat_mode == LPP_COMPAT_3DS) {
            // 3DS mode gets gamepad D-Pad support mapped to keyboard
        ...
        // Triggers - only for compatibility modes
        if (g_compat_mode == LPP_COMPAT_3DS) {
        ...
        if (controllers[0] && SDL_GameControllerGetAttached(controllers[0])) {
            ...
            if (g_gamepad_layout == 0) {
                ...
                if (SDL_GameControllerGetButton(controllers[0], SDL_CONTROLLER_BUTTON_B)) {
                    current_keys[SDL_SCANCODE_SPACE] = true; // Nintendo A (right) = Space
                }
                if (SDL_GameControllerGetButton(controllers[0], SDL_CONTROLLER_BUTTON_A)) {
                    current_keys[SDL_SCANCODE_BACKSPACE] = true; // Nintendo B (bottom) = Backspace
                }
                if (SDL_GameControllerGetButton(controllers[0], SDL_CONTROLLER_BUTTON_Y)) {
                    current_keys[SDL_SCANCODE_Z] = true; // Nintendo X (top) = Z
                }
                if (SDL_GameControllerGetButton(controllers[0], SDL_CONTROLLER_BUTTON_X)) {
                    current_keys[SDL_SCANCODE_X] = true; // Nintendo Y (left) = X
                }
            } else {
                // Xbox layout: Use buttons as-is (no swapping)
                if (SDL_GameControllerGetButton(controllers[0], SDL_CONTROLLER_BUTTON_A)) {
                    current_keys[SDL_SCANCODE_SPACE] = true; // Xbox A (bottom) = Space
                }
                if (SDL_GameControllerGetButton(controllers[0], SDL_CONTROLLER_BUTTON_B)) {
                    current_keys[SDL_SCANCODE_BACKSPACE] = true; // Xbox B (right) = Backspace
                }
                if (SDL_GameControllerGetButton(controllers[0], SDL_CONTROLLER_BUTTON_X)) {
                    current_keys[SDL_SCANCODE_Z] = true; // Xbox X (left) = Z
                }
                if (SDL_GameControllerGetButton(controllers[0], SDL_CONTROLLER_BUTTON_Y)) {
                    current_keys[SDL_SCANCODE_X] = true; // Xbox Y (top) = X
                }
            }
...
// Check gamepad button directly (for native SDL gamepad constants)
static int lua_checkGamepadButton(lua_State *L){
    int argc = lua_gettop(L);
#ifndef SKIP_ERROR_HANDLING
    if (argc != 1 && argc != 2) return luaL_error(L, "wrong number of arguments.");
#endif
    
    int button = luaL_checkinteger(L, 1);  // SDL_CONTROLLER_BUTTON_* constant
    int port = 0;
    if (argc == 2) {
    ...
    // Apply controller layout swapping for A/B and X/Y buttons
    extern int g_gamepad_layout; // 0=Nintendo, 1=Xbox
    int actual_button = button;
    
    if (g_gamepad_layout == 0) {
        // Nintendo layout: Swap buttons to match Nintendo physical layout
        switch (button) {
            case SDL_CONTROLLER_BUTTON_A: actual_button = SDL_CONTROLLER_BUTTON_B; break; // Nintendo A (right) → SDL B
            case SDL_CONTROLLER_BUTTON_B: actual_button = SDL_CONTROLLER_BUTTON_A; break; // Nintendo B (bottom) → SDL A
            case SDL_CONTROLLER_BUTTON_X: actual_button = SDL_CONTROLLER_BUTTON_Y; break; // Nintendo X (top) → SDL Y
            case SDL_CONTROLLER_BUTTON_Y: actual_button = SDL_CONTROLLER_BUTTON_X; break; // Nintendo Y (left) → SDL X
            default: actual_button = button; break; // Other buttons unchanged
        }
    }
...
static const luaL_Reg Controls_functions[] = {
  ...
  {"check",            lua_check},    
  {"checkGamepadButton", lua_checkGamepadButton}, // Direct gamepad button check
...
void luaControls_init(lua_State *L) {
    ...
    // Set key constants (old global version)
    set_key_constants(L);
    
    // Note: Controller initialization is handled by init_controllers() 
    // which is called later after SDL is fully initialized
    ...
    // Register SDL Controller button constants for native gamepad support
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_A); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_A");
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_B); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_B");
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_X); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_X");
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_Y); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_Y");
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_BACK); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_BACK");
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_GUIDE); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_GUIDE");
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_START); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_START");
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_LEFTSTICK); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_LEFTSTICK");
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_RIGHTSTICK); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_RIGHTSTICK");
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_LEFTSHOULDER); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_LEFTSHOULDER");
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_RIGHTSHOULDER); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_RIGHTSHOULDER");
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_DPAD_UP); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_DPAD_UP");
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_DPAD_DOWN); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_DPAD_DOWN");
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_DPAD_LEFT); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_DPAD_LEFT");
    lua_pushinteger(L, SDL_CONTROLLER_BUTTON_DPAD_RIGHT); lua_setglobal(L, "SDL_CONTROLLER_BUTTON_DPAD_RIGHT");
    ...
    lua_pushinteger(L, SDL_SCANCODE_TAB); lua_setglobal(L, "SCE_CTRL_SELECT");   // Select/Back button
    lua_pushinteger(L, SDL_SCANCODE_ESCAPE); lua_setglobal(L, "SCE_CTRL_PSBUTTON"); // Guide/PS button
    ...
    // Backward compatibility: Map 3DS KEY_ codes to SDL scancodes for old 3DS scripts
    // These work with both keyboard and gamepad (gamepad buttons are mapped to keys internally)
    ...
    lua_pushinteger(L, SDL_SCANCODE_TAB); lua_setglobal(L, "KEY_SELECT");        // Select/Back button
    lua_pushinteger(L, SDL_SCANCODE_UP); lua_setglobal(L, "KEY_DUP");            // D-Pad Up
    lua_pushinteger(L, SDL_SCANCODE_DOWN); lua_setglobal(L, "KEY_DDOWN");        // D-Pad Down
    lua_pushinteger(L, SDL_SCANCODE_LEFT); lua_setglobal(L, "KEY_DLEFT");        // D-Pad Left
    lua_pushinteger(L, SDL_SCANCODE_RIGHT); lua_setglobal(L, "KEY_DRIGHT");      // D-Pad Right
    lua_pushinteger(L, 1000); lua_setglobal(L, "KEY_TOUCH");                     // Touch (special value)
}