...
#  endif
		} else if (raw_enc == 49) { // Codec is GSM 6.10 (0x31 in hex, 49 in decimal)
#  ifdef USE_AUDIO_RESAMPLER
			return std::unique_ptr<AudioDecoder>(new AudioResampler(new GsmDecoder()));
#  else
			return std::unique_ptr<AudioDecoder>(new GsmDecoder());
#  endif
		}
	}
